  ...
  <div class="p-4 gap-3">
      <div class=" p-4 bg-white rounded shadow-md">
              <div class="flex justify-between">
                  <p class="m-2 text-lg font-semibold ">Organisations Finance Records</p>
                      <!-- Floating Download Button -->
                      <div class="fixed z-20 bottom-8 right-8">
                        <button class="bg-red-500 hover:bg-blue-600 rounded-full p-4 shadow-lg text-white flex items-center space-x-2">
                  ...
                  <div>
                    <!-- Add Button -->
                    <svg style="color: white; height:36px; width:36px; margin: 12px;" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" @click="openAddModal"><path fill="black" d="M512 64a448 448 0 1 1 0 896 448 448 0 0 1 0-896zm-38.4 409.6H326.4a38.4 38.4 0 1 0 0 76.8h147.2v147.2a38.4 38.4 0 0 0 76.8 0V550.4h147.2a38.4 38.4 0 0 0 0-76.8H550.4V326.4a38.4 38.4 0 1 0-76.8 0v147.2z"></path></svg>
                
                    <!-- Modal -->
                    <div v-if="showAddModal" class="z-10 pt-64 ml-7 absolute inset-0 flex items-center justify-center">
                      <div class="bg-white rounded-lg shadow-md p-5 overflow-y-auto">
                        ...
                        <form v-else @submit.prevent="addOrgFinanceSubmit()" class=" grid grid-cols-2 gap-12 bg-white shadow-md rounded px-8 py-6 mb-4">
                          <!-- Membership Number -->
                          <div class="mb-4">
                          ...
                          <div class="mb-4">
                            ...
                            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                              id="Local" type="text"  value="St Andrews" disabled required>
                          </div>
                        <!-- Description -->
                        <div class="mb-4">
              ...
              <div v-if="editModal" class="z-10 ml-40 pt-72 absolute inset-0 flex items-center justify-center">
                <div class="bg-white rounded-lg shadow-md p-5 overflow-y-auto">
                  ...
                  <p v-if="loading">Processing the payment request</p>
                        <form v-else @submit.prevent="updateOrgFinanceSubmit(finance.financeID)" class=" grid grid-cols-2 gap-12 bg-white shadow-md rounded px-8 py-6 mb-4">
                          <!-- Membership Number -->
                          <div class="mb-4">
                            <label class="block text-gray-700 text-sm font-bold mb-2" for="membershipNumber">Membership Number:</label>
                            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
...
export default {
  ...
  methods:{
  async fetchAndOrgFinance(){
    this.loading = true;
    const URL = "http://localhost:8080/organisationsFinance/getAllOrganisationsByLocal?local=st%20andrews";
  ...
  async fetchOrgFinanceByID(id){
          this.loading = true;
    const URL = `http://localhost:8080/organisationsFinance/getOrganisationsFinanceByFinanceID/${id}`;
    // const token = localStorage.token;
    await axios.get(URL,{
      headers: {'Content-Type': 'application/json',
          // Authorization : 'Bearer ' + token,
  ...
  async addOrgFinanceSubmit(){
        ...
        try{
        await axios.post('http://localhost:8080/organisationsFinance/addOrganisationsFinance',{
  ...
  async handleOption (_option) {
      if(_option = 'yes'){
        try{
        await axios.delete('http://localhost:8080/organisationsFinance/deleteOrganisationsFinance/' + this.FID,{
            headers: {'Content-Type': 'application/json'},
            credentials: 'include'
          }).then((response) =>{
          const data = response.data;
  ...
  async updateOrgFinanceSubmit(id){
        ...
        // make API call or submit form data here
        try{
        await axios.put(`http://localhost:8080/organisationsFinance/updateOrganisationFinanceByFinanceID/${id}`,{
